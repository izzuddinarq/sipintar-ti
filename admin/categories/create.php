    ...
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) $error = 'Sesi tidak valid. Silakan coba lagi.';
        ...
        if ($name === '' || strlen($name) > 100) $error = 'Nama kategori wajib diisi dan maksimal 100 karakter.';
            ...
            if (mysqli_stmt_execute($stmt)) {
                save_log($conn, $_SESSION['user_id'], 'CREATE', 'categories', mysqli_insert_id($conn), 'Menambahkan kategori');
                redirect_to('admin/categories/index.php?success=' . urlencode('Kategori berhasil ditambahkan.'));
            }
...
<div class="admin-wrapper">
    ...
    <main class="main-content">
        <div class="page-header"><div class="header-row"><div><h1>Tambah Kategori</h1><p>Buat kategori baru untuk mengelompokkan barang.</p></div><a href="<?= e(base_url('admin/categories/index.php')); ?>" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> Kembali</a></div></div>
